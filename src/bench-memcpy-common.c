	     ...
	     size_t len)
{
  ...
  if (memcpy (dst, src, len) != MEMCPY_RESULT (dst, len))
    {
      fprintf (stderr, "Wrong return in function %p %p",
	     memcpy(dst, src, len), MEMCPY_RESULT (dst, len));
      ret = 1;
      return;
    }
  ...
  if (memcmp (dst, src, len) != 0)
    {
      fprintf (stderr,"Wrong result in function %p, %p\n", dst, src);
	for (i = 0; i < len; i++)
	  {
	    fprintf (stdout, "\ti = %d, [%p] = %x, [%p] = %x\n", i, src+i, src[i], dst+i, dst[i]);
	    fflush (stdout);
	  }
...
#ifndef ACOND
#define ACOND(A,B,C) 1

#endif
#ifndef SCOND
#define SCOND(A,B,C) 1
...
do_test (size_t align1, size_t align2, size_t len)
{
  ...
  if (SCOND(len) && ACOND(align1, align2, len))
    {
